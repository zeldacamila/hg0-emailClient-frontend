...
import { CheckCircleFilled, EyeFilled, ReloadOutlined, SendOutlined, UserOutlined } from '@ant-design/icons';
...
const MailInboxList: FC<MailInboxListProps> = ({
    ...
    const { data, isLoading, refetch } = useMailsByRecipientQuery(user?.email || '',{
    ...
    return (
        <List<Mail> 
            ...
            renderItem={({id, status, subject, sender, timestamp}) => (
                ...
                >
                    <Item.Meta
                        avatar={<Avatar style={{ backgroundColor: status ? 'green': 'gray' }} icon={<UserOutlined />} />}
                        title={subject}
                        description={
                            <Space size={15}>
                                {sender.email}
